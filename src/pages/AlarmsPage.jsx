...
import {
  ...
  generateKpiAlarms,
  resolveKpiAlarm,
} from "@/features/alarms/services/alarmsService";
...
function isAlarmUpload(upload) {
  return [upload?.remarks, upload?.fileName, upload?.originalName]
    .filter(Boolean)
    .some((value) => String(value).toLowerCase().includes("alarm"));
}
...
export default function AlarmsPage() {
  ...
  const [alarmUploads, setAlarmUploads] = useState([]);
  const [selectedFileId, setSelectedFileId] = useState("");
  const [selectedAlarmFileId, setSelectedAlarmFileId] = useState("");
  const [summary, setSummary] = useState(null);
  const [alarms, setAlarms] = useState([]);
  const [uploadedAlarms, setUploadedAlarms] = useState([]);
  ...
  const [uploadedSeverity, setUploadedSeverity] = useState("");
  ...
  useEffect(() => {
    if (selectedAlarmFileId || viewMode === "uploaded") {
      loadUploadedAlarmData();
    }
  }, [selectedAlarmFileId, uploadedSeverity]);
  ...
  const loadUploads = async () => {
    ...
    const kpiUploads = rows.filter(isKpiUpload);
    const rawAlarmUploads = rows.filter(isAlarmUpload);
    setUploads(kpiUploads);
    setAlarmUploads(rawAlarmUploads);
    if (!selectedFileId && kpiUploads.length > 0) {
      setSelectedFileId(String(kpiUploads[0].id));
    }
    if (!selectedAlarmFileId && rawAlarmUploads.length > 0) {
      setSelectedAlarmFileId(String(rawAlarmUploads[0].id));
  ...
  const loadUploadedAlarmData = async () => {
    setLoadingUploaded(true);
    setUploadedMessage("");
    try {
      const response = await getAlarmData({
        fileId: selectedAlarmFileId || undefined,
        severity: uploadedSeverity,
        search: uploadedSearch,
        page: 1,
        limit: 200,
      });
      if (response?.success) {
        setUploadedAlarms(response.data || []);
        setUploadedTotal(response.pagination?.totalCount || 0);
      } else {
        setUploadedAlarms([]);
        setUploadedTotal(0);
        setUploadedMessage(response?.message || "Uploaded alarm data could not be loaded.");
      }
    } finally {
  ...
  const filteredUploadedAlarms = useMemo(() => {
    const query = uploadedSearch.trim().toLowerCase();
    if (!query) return uploadedAlarms;
    return uploadedAlarms.filter((alarm) =>
      [
        alarm.Circle,
        alarm.Name,
        alarm.EnodeBID,
    ...
    <div className="min-h-screen bg-slate-50 p-4 md:p-6 lg:p-8">
      <div className="mx-auto max-w-[1500px] space-y-6">
        <div className="flex flex-col gap-4 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm md:flex-row md:items-center md:justify-between">
          ...
          <button
            onClick={viewMode === "uploaded" ? loadUploadedAlarmData : loadAlarmData}
            disabled={viewMode === "uploaded" ? loadingUploaded : loading || !selectedFileId}
          ...
          >
            <RefreshCw className={`h-4 w-4 ${(loading || loadingUploaded) ? "animate-spin" : ""}`} />
            Refresh
          </button>
        </div>
        ...
        <div className="inline-flex rounded-xl border border-slate-200 bg-white p-1 shadow-sm">
          <button
            ...
            className={`rounded-lg px-4 py-2 text-sm font-semibold ${viewMode === "uploaded" ? "bg-slate-900 text-white" : "text-slate-600 hover:bg-slate-50"}`}
          >
            Uploaded Alarm Data
          </button>
          <button
            ...
            className={`rounded-lg px-4 py-2 text-sm font-semibold ${viewMode === "generated" ? "bg-slate-900 text-white" : "text-slate-600 hover:bg-slate-50"}`}
          >
            Generated KPI Alarms
          </button>
        </div>
        ...
        {viewMode === "uploaded" ? (
          <>
            <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
              <div className="grid gap-4 lg:grid-cols-[1.4fr_0.7fr_1fr_auto]">
                <div>
                  <label className="mb-2 block text-sm font-medium text-slate-700">Alarm Upload File</label>
                  <select
                    value={selectedAlarmFileId}
                    onChange={(event) => setSelectedAlarmFileId(event.target.value)}
                  ...
                  >
                    <option value="">All alarm uploads</option>
                    {alarmUploads.map((upload) => (
                      <option key={upload.id} value={upload.id}>
                        #{upload.id} - {upload.fileName || "Alarm upload"}
                      </option>
                    ))}
                  </select>
                </div>
                <div>
                  ...
                  <select
                    value={uploadedSeverity}
                    onChange={(event) => setUploadedSeverity(event.target.value)}
                    className="w-full rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm outline-none focus:border-blue-400 focus:ring-2 focus:ring-blue-100"
                  >
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      onKeyDown={(event) => {
                        if (event.key === "Enter") loadUploadedAlarmData();
                      }}
                      placeholder="Search alarm data..."
                      className="w-full rounded-xl border border-slate-200 py-3 pl-10 pr-4 text-sm outline-none focus:border-blue-400 focus:ring-2 focus:ring-blue-100"
                    />
                  </div>
                </div>
                <div className="flex items-end">
                  <button
                    onClick={loadUploadedAlarmData}
                  ...
                  >
                    <RefreshCw className={`h-4 w-4 ${loadingUploaded ? "animate-spin" : ""}`} />
                    Load
                  </button>
                </div>
              </div>
            ...
            <div className="rounded-2xl border border-slate-200 bg-white shadow-sm">
              <div className="flex flex-col gap-3 border-b border-slate-100 p-5 md:flex-row md:items-center md:justify-between">
                <div>
                  <h2 className="text-lg font-semibold text-slate-950">Uploaded Alarm Rows</h2>
                  <p className="text-sm text-slate-500">{number(filteredUploadedAlarms.length)} visible of {number(uploadedTotal)} alarm row(s)</p>
                </div>
              </div>
              ...
              {loadingUploaded ? (
                <div className="flex items-center justify-center py-20 text-slate-500">
                  ...
                  Loading uploaded alarms...
                </div>
              ) : filteredUploadedAlarms.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-20 text-slate-500">
                  <CheckCircle className="mb-3 h-10 w-10 text-emerald-500" />
                  <p className="text-sm font-semibold">No uploaded alarm rows found.</p>
                  <p className="mt-1 text-xs">Upload an alarm file or select another alarm upload.</p>
                </div>
              ) : (
                  ...
                  <table className="min-w-full divide-y divide-slate-100 text-sm">
                    <thead className="bg-slate-50 text-left text-xs font-semibold uppercase text-slate-500">
                      <tr>
                        ...
                        <th className="px-4 py-3">Name / EnodeB</th>
                        <th className="px-4 py-3">Alarm</th>
                        <th className="px-4 py-3">Supplementry Info</th>
                        <th className="px-4 py-3">Alarm Time</th>
                        <th className="px-4 py-3">Aging</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-100">
                      {filteredUploadedAlarms.map((alarm) => (
                        <tr key={alarm.id} className="hover:bg-slate-50">
                          <td className="px-4 py-3">
                            <span className={`rounded-full border px-2.5 py-1 text-xs font-semibold ${severityClasses[String(alarm.Severity || "").toUpperCase()] || severityClasses.WARNING}`}>
                              {alarm.Severity || "Unknown"}
                            </span>
                          </td>
                          <td className="px-4 py-3 font-medium text-slate-800">{alarm.Circle || "-"}</td>
                          <td className="px-4 py-3">
                            <div className="font-semibold text-slate-900">{alarm.Name || "-"}</div>
                            <div className="text-xs text-slate-500">{alarm.EnodeBID || "-"}</div>
                          </td>
                          <td className="px-4 py-3">
                            <div className="font-semibold text-slate-900">{alarm.AlarmNumber || "-"}</div>
                            <div className="max-w-md text-xs text-slate-600">{alarm.AlarmText || "-"}</div>
                          </td>
                          <td className="max-w-md px-4 py-3 text-xs text-slate-600">{alarm.SupplementryInfo || "-"}</td>
                          <td className="px-4 py-3 text-slate-600">{formatDate(alarm.AlarmTime)}</td>
                          <td className="px-4 py-3 text-slate-600">{alarm.StandardAging || "-"}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
