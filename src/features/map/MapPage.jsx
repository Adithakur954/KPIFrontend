...
export default function MapPage() {
  ...
  const runLbPredictionOnMap = useCallback(async () => {
    ...
    const useSampleLbFallback = (message) => {
    ...
    try {
      ...
      if (response?.success) {
        ...
        useSampleLbFallback("Using bundled sample KPI rows because backend has no parsed KPI records for this upload.");
      } else {
      ...
      if (isSampleSiteMapKpiUpload(selectedUpload)) {
        useSampleLbFallback("Using bundled sample KPI rows because backend LB/WCF was not available.");
      } else {
